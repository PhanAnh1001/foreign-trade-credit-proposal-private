...
def timed_node(node_name_or_func=None):
    ...
    Supports both bare and called usage:
        ...
        def my_node(state): ...
    """
    def decorator(func: Callable, _override_name: str | None = None) -> Callable:
        _name = _override_name or func.__name__
        icon = _NODE_ICONS.get(_name, "⚙️")
        _log = get_logger(_name)

        @functools.wraps(func)
        def wrapper(*args, **kwargs) -> Any:
            _log.info(f"{icon} START  ->  {_name}")
            t0 = time.perf_counter()
            try:
                result = func(*args, **kwargs)
                elapsed = time.perf_counter() - t0
                _log.info(f"{icon} END    <-  {_name}  [{elapsed:.2f}s]")
                ...
                elapsed = time.perf_counter() - t0
                _log.error(
                    f"{icon} FAILED x  {_name}  [{elapsed:.2f}s]  ->  {exc}",
                    exc_info=True,
                )
                raise
    ...
    if callable(node_name_or_func):
        # Used as @timed_node (bare, no parentheses)
        return decorator(node_name_or_func)
    else:
        # Used as @timed_node() or @timed_node("name")
        _explicit_name = node_name_or_func
        return lambda func: decorator(func, _override_name=_explicit_name)
